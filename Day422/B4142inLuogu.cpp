...
{
    const int size = 2;
    vector<ll> Volumns(size);
    vector<ll> Materials(size);
    ll single_vol;
    ll time_cost;
    ...
    ll total_vol = arr_sum(arr_multiply(Materials, Volumns));
    cout << div_up(div_up(total_vol, single_vol), time_cost) << endl;
    return 0;
}