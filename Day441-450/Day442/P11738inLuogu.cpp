...
private:
    ...
    int run_expression(int begin_pos, int end_pos)
    {
        ...
        /* 输入指令cin >> 变量1 >> 变量2；。 */
        if (Tokens[i].type == T_CIN)
        {
            Token token = Tokens[++i];
            while (i <= end_pos)
            {
                if (token.type != T_SHIFT_RIGHT)
                {
                    throw invalid_argument(">> should be split symbol in cin, not " + token.value + ".");
                }
    ...
    Token run_statement(int pos)
    {
        ...
        else if (token.type == T_FOR)
        {
            ...
            int condition;
            if (condition_begin <= condition_end)
            {
...
{
#ifndef ONLINE_JUDGE
    freopen(argc == 1 ? "B3648.txt" : argv[1], "r", stdin); // 自己调试用
#endif
