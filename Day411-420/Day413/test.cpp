...
private:
    ...
    stack<Token> Nums;           // 所有INT变量
    stack<Token> Ops;            // 所有操作符
    map<string, int *> RefMap;   // 变量引用映射
    ...
    void calculate()
    {
        ...
        if (judge_right_combine(top_op))
        {
            ...
            {
                Nums.push(value_to_token(a == 0 ? 1 : 0));
                return;
            }
            /* +运算符（正号） */
            else if (top_op.type == T_PLUS)
            {
                Nums.push(value_to_token(a));
                return;
            }
            /* -运算符（负号） */
        ...
        else
        {
            ...
            {
                Nums.push(value_to_token(a <= b ? 1 : 0));
                return;
            }
            /* >=运算符 */
            ...
            {
                Nums.push(value_to_token((a && !b) || (!a && b) ? 1 : 0));
                return;
            }
            /* &&运算符 */
            ...
            {
                // cout << A.value << " " << b << endl;
                Nums.push(value_to_token(b ? 1 : 0));
        ...
        while (i <= end_pos)
        {
            ...
            else if (Tokens[i].type == T_IDENTIFIER)
            {
                    ...
                    Tokens[i + 1].type == T_LPAREN)
                {
                    ...
                    Nums.push({T_INT_CONSTANT, to_string(run_function(func_name, args))});
                }
                /* 变量情况 */
                else
                {
                    ...
                    // cout << var_str << endl;
                    RefMap[var_str] = vs.get_ref(res.first, res.second);

                    Nums.push({T_IDENTIFIER,
                               var_str});
                }
            }
            ...
            else if (OPERATORS.count(Tokens[i].type) > 0)
            {
                ...
                else
                {
                    ...
                    {
                        /* 遇到优先级低的加入，先把优先级高的搞定。 */
                        if (OpLevel(Ops.top()) < OpLevel(Tokens[i]) ||
                            (OpLevel(Ops.top()) == OpLevel(Tokens[i]) &&
